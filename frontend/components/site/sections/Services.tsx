      ...
      <div className="mx-auto max-w-6xl">
        ...
        <ul className="grid gap-5 sm:grid-cols-3">
          {OFFERINGS.map((offering) => (
            <li
              key={offering.title}
              className="flex h-full flex-col gap-2 rounded-2xl bg-neutral-900/50 p-6"
            >
              <h3 className="text-lg font-semibold tracking-tight text-white">{offering.title}</h3>
              <p className="text-sm leading-relaxed text-neutral-400">{offering.body}</p>
            </li>
          ))}
        </ul>
